...
- Cinq modules câblés sur ce mécanisme à ce jour :
...
- Un `admin`/`rh` a toujours accès complet (`write`) à ces cinq modules via son rôle ; un `manager`/`ag` n'y accède que si la permission lui a été explicitement accordée dans `administration.html` (onglet Utilisateurs).
