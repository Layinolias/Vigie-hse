...
|---|---|---|---|---|
| Avant | *saisie à l'écran* | Référentiels (services, sites, familles de risque…) | Administration → Référentiels | À faire AVANT tout import : les colonnes « Service » des fichiers doivent reprendre exactement ces libellés. |
...
| 01 | `01-agents.xlsx` | Agents (Gestion RH) | Gestion RH → onglet Agents → Importer .xlsx | À charger en premier : les autres modules peuvent ensuite proposer ces agents dans leurs formulaires. Le responsable se désigne par son « Nom Prénom » ; il peut figurer plus bas dans le fichier. |
...
| 05 | `05-analyses-accident.xlsx` | Analyses d'accident | Analyse d'accident → Importer .xlsx | La méthode, la conclusion et les actions correctives se reprennent ; l'arbre des causes lui-même (faits, « pourquoi », diagramme d'Ishikawa) se ressaisit à l'écran — la page le signale à l'import. |
...
- **`05-analyses-accident.xlsx`** — Analyses d'accident : obligatoires Actions › « Action ».
