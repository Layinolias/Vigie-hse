...
- [x] Exports PDF/Excel disponibles sur tous les registres qui en ont l'usage. — fait le 2026-09-23 (détail ci-dessous).
  - **Excel : vérifié le 2026-09-23.** Les 22 boutons d'export des 19 registres, avec RH1, admin et le manager limité à ses services, sur les vraies données de démonstration : chaque classeur est relu après écriture réelle. Aucune cellule `undefined`/`null`/`NaN`/`[object Object]` ; le manager n'exporte que ses services (Registre SST : 4 lignes sur 15) ou n'a pas de bouton. Deux défauts trouvés et corrigés (voir `BUGS-CONNUS.md`) : feuilles vides sans en-têtes, date du nom de fichier en UTC.
  - **PDF : fait le 2026-09-23**, choix de l'utilisateur : une impression générique partout tout de suite, et les mises en page dédiées selon la réponse du préventeur (question 12). Les 18 tableaux de 14 registres qui n'avaient pas d'impression ont un bouton « Imprimer / PDF » (`assets/impression.js`) : le tableau tel qu'affiché, filtres et périmètre compris, sous un en-tête daté. Vérifié par un harnais de 189 contrôles (lignes identiques à l'écran, ni bouton ni champ, colonnes vides retirées, filtres reportés, manager limité à ses services, pas de bouton sans export, nettoyage après impression, convocation intacte) et à l'œil en A4 paysage. Seule la Base documentaire reste sans impression : ses fiches se lisent à l'écran.
- [ ] Site en ligne sur un vrai hébergement, à une URL stable, testé (pas seulement en local).
- [x] Mode démo + page vitrine en place. — vérifié le 2026-09-23 : les trois éléments de V0.5.0 existent (vitrine `index.html` avant connexion, bouton « Réinitialiser les données de démonstration » assumé, avertissement « prototype » sur la vitrine), et les données de démonstration se chargent seules dans chaque module (critère « aucune erreur JS », plus haut).
  - La vitrine était **périmée** : elle présentait « onze outils » alors que l'application en compte dix-neuf, et ne montrait pas le compte `PREV1`. Grille mise à jour dans l'ordre du menu latéral, titre sans nombre (il se périmait à chaque module), compte `PREV1` ajouté ; vérifié à l'œil, chaque carte a son icône.
- [x] Documentation à jour (`ETAT-DU-PROJET.md`, ce fichier, `ROADMAP-MODULES-FUTURS.md`). — vérifié le 2026-09-23, d'abord **mécaniquement** : un script confronte la documentation au code — chaque page et chaque composant décrits (ETAT §6 et `CLAUDE.md`), chaque clé de stockage du code documentée en §8 et chaque clé documentée encore utilisée, chaque permission par module présente en §7, chaque fichier cité existant, chaque décompte de points de checklist exact. Sur la documentation d'avant la passe : **10 écarts** (`dashboard.html` absent de `CLAUDE.md`, neuf clés non documentées dont les données de trois modules entiers — EPI, Inspection/Audit, Produits chimiques) ; après : **0**.
  - Puis **à la relecture**, ce qu'un script ne juge pas : `index.html` décrit comme le cockpit (c'est la vitrine, le cockpit est `dashboard.html`), la méthode de vérification du §13 d'ETAT qui affirmait « pas de Node disponible », l'« état courant » du §12 arrêté au 2026-09-15, le point de reprise en tête de la roadmap arrêté au 2026-09-14 (il annonçait six modules livrés depuis comme « en attente »), et les noms des fichiers `DATATEST/` (préfixes `IMPORT`/`REF` absents de la documentation de génération). Tout est corrigé.
