...
- [ ] **Types de document** : dans le panneau « Types de document », ajouter « Fiche de poste » — il apparaît aussitôt dans le formulaire et dans le filtre ; le ressaisir (même en minuscules) est refusé comme doublon. Un type **utilisé** par un document affiche « (utilisé) » et n'a pas de croix de retrait ; retirer un type inutilisé le fait disparaître des listes sans toucher aux documents — **recharger la page : il ne revient pas**. Le dernier type restant ne peut pas être retiré.
