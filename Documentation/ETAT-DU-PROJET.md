...
| `assets/dates-locales.js` | Composant partagé (2026-09-23) — `VigieDates.lire("AAAA-MM-JJ")` (minuit local), `VigieDates.iso(date)` ("AAAA-MM-JJ" local), `VigieDates.aujourdhui()`. Utilisé par `assets/echeance.js`, par le calcul de renouvellement de `epi-dotation.html` et par la date des noms de fichiers d'export de 16 pages. Voir le piège « dates en UTC » au §11. |
...
- **État courant (2026-09-23)** : checklist QA V1 à 120 points, à dérouler de nouveau en direct ; aucune question métier en attente. Restent avant le gel `v1.0.0` : le test en conditions réelles sur l'hébergement et la présentation à un professionnel externe — voir `PLAN-VERSIONS-V1.md`. Point ouvert connu : les « aujourd'hui » calculés en UTC (`BUGS-CONNUS.md`).
...
4. **À l'œil** : le panneau navigateur de l'application ouvre un fichier local sans stockage ; servir le dossier en `http://localhost` (petit serveur statique Node) pour tester une page avec une session.
