...
- **Hors périmètre (assumé)** : dépôt de vrais fichiers (attendre le backend, §14 de `ETAT-DU-PROJET.md`), autres liens contextuels (produit chimique → sa fiche, module de suivi → article pertinent), mise en forme riche des articles, versionnement de l'historique d'un article, commentaires. Les 3 copies inline de `RISK_TAXONOMY_DEFAULT` (`administration.html`, `document-unique.html`, `saisie-duerp.html`) n'ont pas encore été repointées sur `assets/risques-ref.js`.
