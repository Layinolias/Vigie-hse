...
- **33 fiches « Familles de risque »** amorcées (always-merge) avec les **définitions courtes déjà présentes** dans la taxonomie du Document Unique (`assets/risques-ref.js`, source partagée) — **aucun contenu réglementaire inventé** : elles sont à compléter et à valider par le préventeur. Non supprimables (le stockage prime et les ressusciterait) mais masquables en brouillon ; « Rétablir le texte d'origine » disponible.
...
- **Registre AT/MP relié le 2026-09-23** : la colonne « Risque » porte la même pastille « fiche ». Le registre ayant sa propre liste de 9 risques (issue du fichier source), 7 sont rattachés — 5 par le nom, 2 par une correspondance sans ambiguïté (`EQUIVALENCES` dans `assets/risques-ref.js`). « Chute d'objet » et « Déplacement », restés d'abord sans lien, ont été rattachés le même jour sur réponse du préventeur (**Q11**, « rattacher les deux ») : les 9 risques ouvrent désormais une fiche.
- **Liens contextuels livrés le 2026-09-20** : depuis le **Document Unique**, chaque ligne dont la famille de risque est connue porte une pastille « fiche » qui ouvre l'article correspondant ; depuis le **formulaire d'évaluation**, choisir une famille fait apparaître « Lire la fiche de ce risque ». L'identifiant de fiche a une source unique (`assets/risques-ref.js`) et la correspondance se fait par slug, donc un libellé importé sans tiret retrouve quand même sa fiche. Une famille ajoutée à la main dans les référentiels n'a pas de fiche : aucun lien n'est affiché plutôt qu'un lien mort.
- **Complété le 2026-09-21 (réponse Q10 du préventeur)** :
  - **Trame d'une fiche** en quatre sections — *Définition · Exemples de situation · Le danger · Mesures de prévention*. La définition d'origine alimente la première ; les trois autres restent vides et affichent « À compléter par le préventeur » sur les fiches de famille de risque, **c'est lui qui les rédige** (le logiciel fournit la structure, pas le contenu). Un article libre n'affiche que les sections qu'il a. La recherche couvre les quatre sections.
  - **Types de document créés par le préventeur lui-même** (`vigie_hse_doc_types`) : la liste s'édite **dans la page**, accessible à tout compte rédacteur — et non dans Administration, réservée aux administrateurs. Les 10 types livrés (politique HSE, procédure, consigne de sécurité, note de service, arrêté, flash sécurité, fiche métier, texte officiel, compte-rendu, autre) sont des **exemples permanents** : marqués « exemple », non supprimables, toujours rétablis au chargement — choix de l'utilisateur le 2026-09-21, pour que la liste garde une base de départ quoi qu'il arrive. Seuls les types ajoutés se retirent (et pas ceux qu'un document utilise) ; un type retiré reste affiché sur les documents qui le portent.
  - **Afficher / masquer un document** : le préventeur choisit ce que les agents voient. Un document masqué disparaît de la liste, des compteurs et de la tuile KPI côté lecteur, et porte une pastille « Masqué » côté rédacteur. C'est distinct d'« Obsolète », qui qualifie le document lui-même.
  - **Fiches par métier/poste** : couvertes par les types de document (« fiche métier » livré par défaut), le préventeur ayant la main sur les types à créer.
- **Hors périmètre (assumé)** : dépôt de vrais fichiers (attendre le backend, §14 de `ETAT-DU-PROJET.md`), autres liens contextuels (produit chimique → sa fiche, module de suivi → article pertinent), mise en forme riche des articles, versionnement de l'historique d'un article, commentaires.
...
## Pistes gardées en réserve (non planifiées)

Des possibilités identifiées, écartées pour l'instant par le métier, à ne pas perdre de vue.

- **Mises en page dédiées à l'impression** (question 12 du préventeur, 2026-09-23 : « Pas de besoin de plus pour le moment, mais gardez une note pour ne pas oublier que c'est possible »). Aujourd'hui chaque registre imprime la liste telle qu'affichée (`assets/impression.js`) ; seules la convocation médicale, la fiche d'accueil au poste, le Registre AT/MP et le Document Unique ont une mise en page propre. Candidats si le besoin revient : **plan de prévention** d'une entreprise extérieure signé par les deux parties, **compte rendu d'exercice d'évacuation**, **rapport d'analyse d'accident**, **fiche de remise d'EPI** signée par l'agent, **rapport d'inspection**, **fiche de la base documentaire** à afficher. Modèle technique à suivre : `#accueilPrint` (`accueil-poste.html`) ou `#letterPrint` (`sante-visites.html`). Reposer la question au préventeur avant d'en construire une (contenu, mentions, signataires, conservation).
