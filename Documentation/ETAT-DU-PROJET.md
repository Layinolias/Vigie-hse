...
| `penibilite.html` | Pénibilité & fiches individuelles (module 5, 2026-09-25, réponse Q15) — **postes** (intitulé, collectivité, service, description, actif) et, pour chacun, une **campagne d'expositions par an** : durée annuelle d'exposition au-delà de l'intensité minimale pour les sept critères à seuil de l'article D4163-2 (le bruit en compte deux), case « exposé » + précision pour les quatre facteurs sans seuil de L4161-1. Chaque **agent actif** de Gestion RH hérite des expositions de son poste (`posteId`) ; la **suite à donner** dépend de son `statut` : « Déclaration C2P » si le statut est coché dans l'onglet « Seuils & statuts » (aucun ne l'est d'office — question juridique), « Fiche de suivi individuel » sinon, « Statut à renseigner » / « Poste non renseigné » / « Campagne non saisie » pour les cas incomplets. Fiche individuelle imprimable, export `.xlsx` (Postes, Expositions, Agents de l'année). Accès : RH/admin, ou permission granulaire `penibilite`. Pas d'import en V1. |
| `dialogue-social.html` | Dialogue social (module 13, 2026-09-25, réponse Q16 « les deux ») — **questions à la direction** (fil de messages ; statut calculé : « En attente de réponse » tant que le dernier message vient d'un représentant, « Répondue » quand il vient de la direction, « Close » quand la direction la clôt), **réunions de la formation spécialisée** (date, type, ordre du jour, compte rendu, avis rendus et suite donnée), **visites de site** dont les actions rejoignent le Plan d'actions (origine « Dialogue social »), **indicateurs anonymisés** (accidents de l'année, par famille de risque et par service — un effectif de 1 ou 2 s'affiche « moins de 3 » —, observations du registre SST, risques du DUERP, actions ; jamais de nom ni de donnée de santé). Direction = comptes RH/admin ; représentants = permission granulaire `dialogue-social` (lecture ou écriture). |
...
| `assets/penibilite-ref.js` | Composant partagé (2026-09-25) — `VigiePenibilite.CRITERES` (sept critères à seuil, article D4163-2 en vigueur depuis le 1er septembre 2023, relu sur Légifrance le 2026-09-25 et recoupé avec service-public), `SANS_SEUIL` (quatre facteurs de L4161-1), `evaluer(valeurs)` (critères et facteurs atteints — un seuil est une durée **minimale**, atteint dès qu'égalé), `SOURCE` (texte, lien, date de relecture). **Les seuils changent par décret : les relire à chaque version.** |
...
modulePermissions: {
  ...
  "penibilite": "read" | "write",        // Pénibilité (postes, expositions, statuts ouvrant droit au C2P)
  "dialogue-social": "read" | "write",   // Dialogue social (représentants du personnel ; la direction = RH/admin)
...
- Dix modules câblés sur ce mécanisme à ce jour :
  ...
  - `penibilite.html` (clé `"penibilite"`, read/write) — module 5, voir `ROADMAP-MODULES-FUTURS.md`.
  - `dialogue-social.html` (clé `"dialogue-social"`, read/write) — module 13, voir `ROADMAP-MODULES-FUTURS.md`.
- **Compte anonymisé** (2026-09-25, réponse Q16) : case `u.anonymise` dans Administration, copiée dans la session. En mode serveur, `serveur/anonymisation.js` ne transmet à ce compte aucune donnée de santé (les cinq clés `sensible` autres que le registre AT/MP retirées ; `vigie_hse_dataset` réduit à id, collectivité, service, mois, année, type, arrêt, jours d'arrêt, famille de risque — y compris dans une réponse de conflit) ; il n'y peut qu'ajouter, greffé sur le registre complet. En mode navigateur, seul l'affichage du module l'est (avertissement à l'écran).
...
| `vigie_hse_postes` | Postes de travail (module 5) : `{ id, intitule, collectivite, service, description, actif, auteur, dateCreation }`. Un agent de `vigie_hse_agents` y est rattaché par son champ `posteId` (facultatif), à côté de son `statut` (liste `statutsRH` des référentiels). |
| `vigie_hse_expositions` | Campagnes annuelles d'exposition par poste : `{ id: "exp-<posteId>-<année>", posteId, annee, valeurs: { hyperbare, temperatures, bruitMoyen, bruitCrete, nuit, alternantes, repetitif }, sansSeuil: { manutention|postures|vibrations|chimiques: { expose, precision } }, commentaire, auteur, dateSaisie }`. Identifiant stable par poste et par année : deux saisies simultanées se fusionnent (mode serveur). |
| `vigie_hse_penibilite_parametres` | `{ statutsC2P: [...] }` — statuts cochés comme ouvrant droit au compte professionnel de prévention (vide par défaut). |
| `vigie_hse_ds_questions` | Questions à la direction (module 13) : `{ id, objet, auteur, date, close, closePar, closeLe, messages: [{ id, auteur, direction: bool, texte, date }] }`. |
| `vigie_hse_ds_reunions` | Réunions de la formation spécialisée : `{ id, date, type, ordreDuJour, compteRendu, avis: [{ id, sujet, avis, suite }], auteur, dateCreation }`. |
| `vigie_hse_ds_visites` | Visites de site : `{ id, date, site, collectivite, service, participants, constats, actions: [{ id, description, responsable, echeance, statut }] }` — actions reprises par `plan-actions.html` (id `pa-ds-<visite>-<action>`). |
...
- **2026-09-25** : réponses Q15 et Q16 reçues ; module 5 **Pénibilité** livré (`penibilite.html`, poste et statut ajoutés aux agents de Gestion RH). Reste le module 13 (Dialogue social).
